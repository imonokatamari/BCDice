...
INFO_MESSAGE_TEXT
  ...
  def rollTableCommand(command)
    ...
    when /WDT2/i
      type = "世界歪曲表ver2.0"
      output, total_n = mm_world_distortion_table_ver2()
    when /WDT/i
      type = "世界歪曲表"
      output, total_n = mm_world_distortion_table()
    when /DTO/i
      type = "歪み表(野外)"
      output, total_n = mm_distortion_table_outdoor()
    when /DTS/i
      type = "歪み表(海)"
      output, total_n = mm_distortion_table_sea()
    when /DT2/i
      type = "歪み表ver2.0"
  ...
  # 世界歪曲表ver2.0(d66)[WDT2]
  def mm_world_distortion_table_ver2
    table = [
  ...
  def mm_omens_table_ver2
    table = [
      ...
      "【肉体の崩壊】\n硝子の砕ける音がして、あなたも砕けていく。あなたは2D6+キャラクターレベル点の【HP】を失う。",
      "【放心】\nあなたの心に大きな衝動が襲いかかる。心は伽藍に向かって歪み、瞳は色のない悪夢を見る。あなたはバッドステータスの[放心を受ける。",
      "【放心】\nあなたの心に大きな衝動が襲いかかる。心は伽藍に向かって歪み、瞳は色のない悪夢を見る。あなたはバッドステータスの[放心を受ける。",
  ...
  # 歪み表(野外)(d66)[DTO]
  def mm_distortion_table_outdoor
    table = [
